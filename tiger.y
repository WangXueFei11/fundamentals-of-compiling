...
/* decs_nonempty */
decs_nonempty :  decs_nonempty_s {$$ = new absyn::DecList($1);}  |  decs_nonempty_s decs_nonempty {$$ = $2; $$->Prepend($1);};
/* dec */
decs_nonempty_s :  tydec {$$ = new absyn::TypeDec(scanner_.GetTokPos(), $1);}
  |  vardec {$$ = $1;}  |  fundec {$$ = new absyn::FunctionDec(scanner_.GetTokPos(), $1);};
/* tydec */
tydec :  tydec_one {$$ = new absyn::NameAndTyList($1);}  |  tydec_one tydec {$$ = $2; $$->Prepend($1);};
/* type type-id = ty */
tydec_one :  TYPE ID EQ ty {$$ = new absyn::NameAndTy($2, $4);};
/* vardec */
vardec :  VAR ID ASSIGN exp {$$ = new absyn::VarDec(scanner_.GetTokPos(), $2, nullptr, $4);}    |  VAR ID COLON ID ASSIGN exp {$$ = new absyn::VarDec(scanner_.GetTokPos(), $2, $4, $6);};
/* fundec */
fundec :  fundec_one {$$ = new absyn::FunDecList($1);}  |  fundec_one fundec {$$ = $2; $$->Prepend($1);};
...
/* tyfields */
tyfields :  tyfields_nonempty {$$ = $1;}   |  {$$ = new absyn::FieldList();};
tyfields_nonempty :  tyfield {$$ = new absyn::FieldList($1);}  |  tyfield COMMA tyfields_nonempty {$$ = $3; $$->Prepend($1);};
...
rec :  rec_nonempty {$$ = $1;}   |  {$$ = new absyn::EFieldList();};
rec_nonempty :  rec_one {$$ = new absyn::EFieldList($1);}   |  rec_one COMMA rec_nonempty {$$ = $3; $$->Prepend($1);};
